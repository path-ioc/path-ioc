...
#### 1. [从图论视角看 DI：为什么依赖注入无法实现拓扑并发，只能搞串行和三级缓存补丁？](/zh/articles/why-di-cannot-concurrent)
...
#### 2. [单线程事件循环下的容器双态：客户端全局单例 vs 服务端请求隔离与闭包重型缓存](/zh/articles/client-vs-server-container-patterns)
...
#### 3. [初始化权衡决策：懒连接推迟 vs 拓扑预热，如何彻底终结“异步染色蔓延”？](/zh/articles/async-preheat-vs-lazy-connection)
...
#### 4. [“老牌 JS IoC 框架搞了这么多概念，总有一个优势值得借鉴吧？”——Path-IoC 的极简架构立场](/zh/articles/rethinking-legacy-ioc-concepts)
...
#### 5. [架构反模式：为什么在业务依赖中硬编码全路径是错的？](/zh/articles/anti-pattern-full-path)
...
* **如果您是 Java / Spring 架构师**：推荐先阅读 [从图论视角看 DI：为什么依赖注入无法实现拓扑并发？](/zh/articles/why-di-cannot-concurrent)，再阅读主线 [《架构宣言：向 Spring 致敬与动态语言正解》](/zh/guide/architecture-manifesto)。
...
* **如果您在做全栈架构与函数设计**：推荐深入研读 [初始化权衡决策：懒连接 vs 拓扑预热](/zh/articles/async-preheat-vs-lazy-connection) 与 [单线程事件循环下的容器双态](/zh/articles/client-vs-server-container-patterns)。
